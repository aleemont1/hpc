...
int make_hist(const char *text, int hist[ALPHA_SIZE])
{
    ...
    const int N_THR = omp_get_num_threads();
    int local_hist[N_THR][ALPHA_SIZE];
    /* Reset the histogram */
    for (i = 0; i < N_THR; i++)
    {
...
#pragma omp parallel reduction(+ : nlet) default(none) private(i) shared(text_size, local_hist, text, N_THR)
    {
        /* Count occurrences */
        const int my_id = omp_get_thread_num();
        const int my_start = (text_size * my_id) / N_THR;
        const int my_end = (text_size * (my_id + 1)) / N_THR;
        printf("[T%d] start: %d end: %d", my_id, my_start, my_end);
        printf("Ok\n\n");
