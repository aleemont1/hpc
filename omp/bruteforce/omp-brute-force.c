...
int main(int argc, char *argv[])
{
  ...
  int found = 0;
  /*char *out = (char *)malloc(msglen); where to put the decrypted message. DEVE ESSSERE UNO PER THREAD, LO SPOSSTO NEL CICLO.
  assert(out != NULL);*/
#pragma omp parallel default(none) shared(n, msglen, KEY_LEN, CHECK_LEN, enc, check, found)

  /**
   * In questo caso posso mettere found come shared, nonostante venga letta e modificata da ogni thread senza protezioni.
   * Tuttavia, found all'inizio vale 0 per tutti, e anche se un thread dovesse modificare found a 1 nell'istante successivo
   * in cui un altro thread ha letto il flag a 0, nella peggiore delle ipotesi viene eseguita una iterazione in più e terminerà
   * a quella successiva. In generale questo sarebbe sbagliato, ma in un caso simile non ci sono problemi di sorta.
   */

  {
    // Ogni thread deve sapere chi è e quante istruzioni deve eseguire nel pool
    const int my_id = omp_get_thread_num();
    const int num_threads = omp_get_num_threads();
    // Suddivido n valori in maniera equa tra i thread.
    const int my_start = (n * my_id) / num_threads;
    const int my_end = (n * (my_id + 1)) / num_threads;
    char key[KEY_LEN + 1];              /* sprintf will output the trailing \0, so we need one byte more for the key. UNO PER OGNI THREAD */
    char *out = (char *)malloc(msglen); /* where to put the decrypted message. UNO PER OGNI THREAD */
    assert(out != NULL);
    for (int k = my_start; k < my_end && !found; k++) // NON SO QUANDO TERMINA => NON POSSO USARE OMP PARALLEL FOR!
    {
