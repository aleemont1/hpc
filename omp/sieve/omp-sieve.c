...
long mark(char *isprime, int k, long from, long to)
{
    ...
    long p_nmarked[max_thr];
    /* [TODO] Parallelize this function */
    from = ((from + k - 1) / k) * k; /* start from the lowest multiple of p that is >= from */
#pragma omp parallel default(none) shared(isprime, from, to, k, p_nmarked)
    {
        const int id = omp_get_thread_num();
        const int num_thr = omp_get_num_threads();
        const long n = (to - from);
        long my_from = from + (n * id) / num_thr;
        assert(my_from > 0);
        my_from = ((my_from + k - 1) / k) * k; /* start from the lowest multiple of p that is >= my_from */
        const long my_to = from + (n * (id + 1)) / num_thr;
        p_nmarked[id] = 0l;
